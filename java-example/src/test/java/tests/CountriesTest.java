...
public class CountriesTest extends TestBase {
  ...
  public void countriesTest() {
    ...
    List<WebElement> notZeroZoneCountries = app.driver.findElements(By.xpath("//tr[@class='row'][.//td[6][text() != '0']]"));//найти строки таблицы со странами, где количество зон отлично от нуля

    int size = notZeroZoneCountries.size();
    for (int i = 0; i < size; i++) {
      WebElement notZeroZoneCountry = app.driver.findElements(By.xpath("//tr[@class='row'][.//td[6][text() != '0']]")).get(i);//взять из списка 1 страну, где количество зон отлично от нуля
      ...
      List<WebElement> zones = app.driver.findElements(By.xpath("//input[contains(@name, 'zones') and contains(@name, '[name]')]"));//получить все зоны со страницы страны
      ...
      app.driver.navigate().to("http://localhost/litecart/admin/?app=countries&doc=countries");//со страницы конкретной страны вернуться к списку всех стран, чтобы повторить цикл
    }
